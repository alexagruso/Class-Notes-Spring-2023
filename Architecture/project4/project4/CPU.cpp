...
void CPU::printFinalStats()
{
    ...
    std::cout << "RAW Hazards: " << stats.getHazards() << '\n';
    std::cout << "Instructions:Hazards: " << instructions / (float)stats.getHazards() << '\n';

    for (int i = ID; i < PIPESTAGES; i++)
    {
        std::cout << "Stage " << i << ": " << stats.getStageHazard(i) << "\t";
        std::cout << setprecision(1);
        std::cout << stats.getStageHazard(i) * 100 / (float)stats.getHazards() << "%\n";
    }
}
